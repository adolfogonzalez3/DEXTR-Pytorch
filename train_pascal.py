...
if __name__ == '__main__':
    ...
    net_preprocess = nn.Conv2d(nInputChannels, 3, kernel_size=2,
                               stride=1, padding=1, bias=False)
    net = models.densenet121(pretrained=True)
    #print(net.classifier)
    #net.classifier = nn.Conv2d(1024, 1, kernel_size=1,
    #                           stride=1, padding=1, bias=False)
    #print(net.classifier)
    net_postprocess = nn.Conv2d(1024, 1, kernel_size=2, stride=1, padding=1)
    if resume_epoch == 0:
    ...
    train_params = [
                    ...
                    {'params': net_preprocess.parameters(), 'lr': 1e-3},
                    {'params': net_postprocess.parameters(), 'lr': 1e-3}
                   ]

    net.to(device)
    net_preprocess.to(device)
    net_postprocess.to(device)
    ...
    if resume_epoch != nEpochs:
        ...
        voc_train = pascal.VOCSegmentation(split='train', download=True, transform=composed_transforms_tr)
        voc_val = pascal.VOCSegmentation(split='val', download=True, transform=composed_transforms_ts)
        ...
        for epoch in range(resume_epoch, nEpochs):
            ...
            for ii, sample_batched in tqdm(enumerate(trainloader)):
                ...
                inputs = net_preprocess.forward(inputs)
                #output = net.forward(inputs)
                output = net.features(inputs)
                output = net_postprocess.forward(output)
                #print(output.shape)
                #print(output.device)
                #print(output.shape)
                output = output.cpu()
