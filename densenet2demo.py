...
net.features[0] = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3,
                            bias=False)
modelName = 'dextr_pascal'
classifier = PSPModule(512, sizes=(1, 2, 3, 6, 12))
first = net.features[0]
net = nn.Sequential(net.features[:-5], classifier)
save_dir_root = pathlib.Path(__file__).resolve().parent
save_dir = save_dir_root / 'run_{:d}'.format(9)
weights_path = save_dir / 'models'
weights_path /= '{}_epoch-{:d}.pth'.format(modelName, 299)
state_dict_checkpoint = torch.load(str(weights_path),
